...
def get_location_images(lat, lng):
    """Get images for a specific location with optional date and transect filtering"""
    try:
        start_date = request.args.get('start_date')
        end_date = request.args.get('end_date')
        transect_filter = request.args.get('transect')  # New transect filter
        scope = request.args.get('scope', 'location')  # location, municipality
        ...
        if scope == 'municipality':
            # First, get the municipality for this location
        ...
        with conn.cursor(cursor_factory=psycopg2.extras.RealDictCursor) as cur:
            ...
            for img in images:
                result.append({
                    ...
                    'municipality': img['municipality'] or '',
                    'barangay': img['barangay'] or '',
                    'transect': img['transect']
                })
