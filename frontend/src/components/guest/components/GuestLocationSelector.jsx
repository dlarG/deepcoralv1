...
import "../../admin/styles/locationSelector.css";
...
const GuestLocationSelector = ({
  ...
  // Function to load transect counts for a specific location
  ...
  // Function to check if a transect is available
  ...
  // Function to get transect status message
  ...
  // FIXED: Load existing locations without infinite loop
      ...
      if (data.locations && Array.isArray(data.locations)) {
        ...
        const processedLocations = data.locations.map((loc, index) => {
          return {
            ...
            contributor_count: 1,
            transects: loc.transects || [],
            location_id: getLocationDisplayName(loc, index),
          };
        });
  ...
  // FIXED: Load locations and count images only once when modal opens
  useEffect(() => {
    if (isOpen) {
  ...
  }, [isOpen]); // Remove other dependencies to prevent loops
  ...
  // FIXED: Guest-specific save function
  const handleSaveLocation = async () => {
    ...
    try {
      ...
      // FIXED: Extract image IDs from guest batch results
      let imageIds = [];
      ...
      if (imageIds.length === 0) {
        ...
        alert("No analyzed images found to save with location.");
        return;
      }
      ...
      if (response.ok) {
        ...
        setExistingLocations([]); // Clear to force reload
      ...
      <div className="location-selector-modal">
        <div className="location-header">
          <div className="header-left">
            <FiMap size={24} />
            <div>
              <h2>Select Location for Guest Coral Data</h2>
              <p>Choose where these analyzed coral images were captured</p>
