...
.gradient-overlays {
  ...
  right: 0;
  bottom: 0;
  background: linear-gradient(135deg, rgba(14, 165, 233, 0.9) 0%, rgba(6, 182, 212, 0.8) 100%);
}
...
.profile-card.personal-card::before {
  background: linear-gradient(135deg, #0ea5e9 0%, #06b6d4 100%);
}

.profile-card.account-card::before {
  background: linear-gradient(135deg, #10b981 0%, #059669 100%);
}

.profile-card.security-card::before {
  background: linear-gradient(135deg, #8b5cf6 0%, #7c3aed 100%);
}

.profile-card.actions-card::before {
...
.summary-card.pending-approved .card-icon {
  background: #ecfdf5;
  color: #10b981;
}

.summary-card.ready .card-icon {
  background: #fffbeb;
  color: #f59e0b;
}
...
.bulk-analyze-btn, .bulk-location-btn {
  display: flex;
  align-items: center;
  gap: 0.5rem;
  padding: 0.75rem 1.5rem;
...
.bulk-analyze-btn:hover:not(:disabled) {
  transform: translateY(-1px);
  box-shadow: 0 4px 12px rgba(59, 130, 246, 0.3);
}

.bulk-analyze-btn:disabled {
  opacity: 0.6;
  cursor: not-allowed;
}
...
.pending-approved-card:hover {
  border-color: #cbd5e1;
  transform: translateY(-2px);
  box-shadow: 0 8px 20px rgba(0, 0, 0, 0.08);
}

.pending-approved-card.selected {
  border-color: #3b82f6;
  background: #eff6ff;
}

.pending-approved-card.list {
  display: flex;
  gap: 1.5rem;
  align-items: center;
}

.pending-approved-card.list .image-preview {
  width: 120px;
  height: 120px;
  flex-shrink: 0;
}

.pending-approved-card.list .image-info {
  flex: 1;
}

.pending-approved-card.list .analysis-results {
  flex: 2;
}

.pending-approved-card.list .image-actions {
  align-self: stretch;
  justify-content: center;
}
