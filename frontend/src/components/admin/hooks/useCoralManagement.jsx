...
export default function useCoralManagement() {
    ...
    try {
      ...
      if (coralModalMode === "add") {
        response = await axios.post(
          `${process.env.REACT_APP_API_URL}/admin/corals`,
          formData,
          {
            headers: {
        ...
        response = await axios.put(
          `${process.env.REACT_APP_API_URL}/admin/corals/${currentCoral.id}`,
          formData,
          {
            headers: {
              "X-CSRF-Token": csrfToken,
      ...
      console.error("Admin coral operation failed:", error);
      ...
      if (error.response?.status === 500) {
        ...
        showErrorModal(
          "Access Denied",
          "You don't have permission to perform this action. Please check your admin privileges."
        );
  ...
  const handleDeleteCoral = async (coralId) => {
    ...
    try {
      const csrfToken = await fetchCsrfToken();
      await axios.delete(
        `${process.env.REACT_APP_API_URL}/admin/corals/${coralId}`,
        {
